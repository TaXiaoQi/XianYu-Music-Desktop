                  ...
                  <div class="space-y-3">
                    ...
                    <section class="rounded-xl border border-gray-200/70 bg-white/40 p-3 transition-all hover:border-[#EC4141]/40 dark:border-white/10 dark:bg-white/5">
                      ...
                      <div v-show="store.flangerEnabled" class="mt-2 space-y-1.5">
                        <div class="flex items-center gap-2"><span class="w-16 shrink-0 text-[12px] text-gray-600 dark:text-gray-300">速率</span><input type="range" class="fx-slider" min="1" max="50" v-model.number="store.flangerRate"><span class="w-8 shrink-0 text-right text-[11px] tabular-nums text-gray-500 dark:text-gray-400">{{ store.flangerRate.toFixed(1) }}Hz</span></div>
                        <div class="flex items-center gap-2"><span class="w-16 shrink-0 text-[12px] text-gray-600 dark:text-gray-300">深度</span><input type="range" class="fx-slider" min="5" max="50" v-model.number="store.flangerDepth"><span class="w-8 shrink-0 text-right text-[11px] tabular-nums text-gray-500 dark:text-gray-400">{{ (store.flangerDepth / 10).toFixed(1) }}ms</span></div>
                        <div class="flex items-center gap-2"><span class="w-16 shrink-0 text-[12px] text-gray-600 dark:text-gray-300">反馈</span><input type="range" class="fx-slider" min="0" max="90" v-model.number="store.flangerFeedback"><span class="w-8 shrink-0 text-right text-[11px] tabular-nums text-gray-500 dark:text-gray-400">{{ store.flangerFeedback }}%</span></div>
                        <div class="flex items-center gap-2"><span class="w-16 shrink-0 text-[12px] text-gray-600 dark:text-gray-300">混合</span><input type="range" class="fx-slider" min="0" max="100" v-model.number="store.flangerMix"><span class="w-8 shrink-0 text-right text-[11px] tabular-nums text-gray-500 dark:text-gray-400">{{ store.flangerMix }}%</span></div>
                      </div>
                    </section>
