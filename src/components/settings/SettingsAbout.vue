...
<template>
  <div class="flex min-h-full min-w-0 flex-col items-center pb-8">
    ...
    <div class="flex max-w-full flex-nowrap items-center justify-center gap-2.5">
      <button
        v-if="aboutConfig.updateEnabled"
        type="button"
        :disabled="isCheckingUpdate"
        @click="checkUpdateManual"
        class="flex shrink-0 items-center gap-2 whitespace-nowrap rounded-xl bg-[#EC4141] px-5 py-2.5 text-sm font-medium text-white shadow-lg shadow-red-500/20 transition active:scale-95 hover:bg-[#d13a3a] disabled:cursor-not-allowed disabled:opacity-70"
      >
        <svg v-if="isCheckingUpdate" class="h-4 w-4 shrink-0 animate-spin" viewBox="0 0 24 24" fill="none">
        ...
        {{ isCheckingUpdate ? '检查中...' : aboutConfig.updateText }}
      </button>

      <a
        v-if="aboutConfig.officialSiteUrl"
        :href="aboutConfig.officialSiteUrl"
        target="_blank"
        rel="noreferrer"
        class="flex shrink-0 cursor-pointer items-center gap-2 whitespace-nowrap rounded-xl bg-[#EC4141] px-5 py-2.5 text-sm font-medium text-white no-underline shadow-lg shadow-red-500/20 transition active:scale-95 hover:bg-[#d13a3a]"
      >
      ...
      <a
        ...
        :href="aboutConfig.referenceProjectUrl"
        target="_blank"
        rel="noreferrer"
        class="flex shrink-0 cursor-pointer items-center gap-2 whitespace-nowrap rounded-xl bg-white/20 backdrop-blur-md border border-gray-200/40 px-5 py-2.5 text-sm font-medium text-gray-800 no-underline transition active:scale-95 shadow-sm hover:bg-white/30 hover:border-[#EC4141]/35 dark:bg-black/10 dark:border-gray-800/40 dark:text-white dark:hover:bg-white/10"
      >
