...
interface CreateLibraryFolderTreeDeps {
...
export const createLibraryFolderTree = ({
  addLibraryFolderPath,
  removeLibraryFolderPath,
  showToast,
}: CreateLibraryFolderTreeDeps) => {
  const libraryStore = useLibraryStore();
  const { folderTree } = storeToRefs(libraryStore);

  const resolveFolderNode = (target: string | FolderNode) =>
    typeof target === 'string' ? findNode(folderTree.value, target) : target;

  const ensureFolderChildrenLoaded = async (target: string | FolderNode) => {
