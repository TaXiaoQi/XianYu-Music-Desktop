...
function detectBackup(data: any): DetectedBackup {
  ...
  if (Array.isArray(data?.data?.musicSheets)) {
    return {
      format: 'bakamusic',
      sheets: data.data.musicSheets,
      version,
      restoreStringifiedIds: version === STRINGIFIED_TRACK_ID_BACKUP_VERSION,
    };
  }
  // MusicFree: musicSheets 位于顶层。其版本号语义与 BakaMusic 无关，不做 ID 迁移。
  if (Array.isArray(data?.musicSheets)) {
    return { format: 'musicfree', sheets: data.musicSheets, version, restoreStringifiedIds: false };
  }
