...
pub fn parse_dsf_info(path: &str) -> Result<DsdInfo, String> {
    ...
    file.read_exact(&mut magic)
        .map_err(|_| "not a DSD file".to_string())?;
    if &magic != b"DSD " {
    ...
    file.read_exact(&mut fmt)
        .map_err(|_| "bad fmt payload".to_string())?;
    ...
    if skip_fmt > 0 {
        file.seek(SeekFrom::Current(skip_fmt as i64))
    ...
    let (data_offset, data_size) = loop {
        ...
        if chunk_id == b"data" {
            ...
            file.read_exact(&mut inner)
                .map_err(|_| "bad data size".to_string())?;
...
pub fn parse_dff_info(path: &str) -> Result<DsdInfo, String> {
    ...
    file.read_exact(&mut frm8_size_buf)
        .map_err(|_| "bad FRM8 size".to_string())?;
    let frm8_size = read_u64_be(&frm8_size_buf);

    let mut form_type = [0u8; 4];
    file.read_exact(&mut form_type)
        .map_err(|_| "bad form type".to_string())?;
    if &form_type != b"DSD " {
...
impl DopStreamSource {
    ...
    pub fn next_frames(&mut self, out: &mut Vec<u8>, max_frames: usize) -> Result<usize, String> {
        ...
        while produced < max_frames {
            ...
            let marker = if self.frame_index & 1 == 0 {
                DOP_MARKER_LOW
            } else {
                DOP_MARKER_HIGH
            };
    ...
    pub fn seek_to_frame(&mut self, target_frame: u64) -> Result<u64, String> {
        match self.format {
            ...
            DsdFormat::Dff => {
                ...
                let byte_off = target_frame.saturating_mul(channels).min(self.data_size);
                self.reader
                    .seek(SeekFrom::Start(self.start_offset + byte_off))
                    .map_err(|e| e.to_string())?;
                self.data_remaining = self.data_size - byte_off;
...
mod tests {
    ...
    fn build_dsf(channels: u32, block_size: u32, dsd_rate: u32, blocks: &[u8]) -> Vec<u8> {
        ...
        out.extend_from_slice(&(data_hdr + 8).to_le_bytes());
    ...
    fn seek_to_frame_skips_to_byte_offset() {
        ...
        let mut out = Vec::new();
        assert_eq!(src.next_frames(&mut out, 8).unwrap(), 4);
        let mut data = Vec::new();
    ...
    fn dff_marker_alternates_across_buffers() {
        ...
        assert_eq!(
            markers,
            vec![0x05, 0xFA, 0x05, 0xFA, 0x05, 0xFA, 0x05, 0xFA]
        );
        let _ = std::fs::remove_file(&path);
    }
    ...
    fn dff_seek_to_exact_frame() {
        ...
        let mut out = Vec::new();
        assert_eq!(src.next_frames(&mut out, 8).unwrap(), 5);
